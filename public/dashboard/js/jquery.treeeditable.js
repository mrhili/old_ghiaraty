/*!
 * bootstrap-
 treetable - jQuery plugin for bootstrapview treetable
 *
...
(function($){
	$.fn.bstreetable = function(options){
		...
		var settings = {
            ...
            text:{
                NodeDeleteText:"هل حقا تريد حذف عذه المعلومة ؟"
            }
		};
		var TREENODECACHE = "treenode";
		var language ={};
		language.addchild = "إضافة فرع جديد";
		if(options) {           
        ...
        /*render data*/
        var dom_addFirstLevel = $("<div class='tt-operation m-b-sm'></div>").append($("<button class='btn btn-primary btn-sm pull-left j-addClass'><i class='fa fa-level-down'></i>&nbsp;إضافة أصل</button>"));
        ...
        for(var i=0;i<settings.data.length;i++){
        	...
        	//render first level row while row.parent_id equals 0 or null or undefined
        	if(!row.parent_id){
        ...
        element.delegate(".j-remove","click",function(event){
            ...
            if(parentDom.attr("data-loaded")=="true"){
                ...
                DELETE MESSAGE CAN NOT BE REMOVABLE
                ...
                */
                ...
                if(parentDom.attr("data-id")){
                    ...
                    for(var i=0;i<settings.data.length;i++){
                        if(settings.data[i].parent_id==parentDom.attr("data-id")){
                            existChild = true;
                            break;
                        }
                    }
        ...
        element.delegate(".j-addChild","click",function(){
            ...
            if(!hasError){
                var parent_id = curElement.find(".j-expend").attr("data-id");
                var curLevel = $(this).parents(".class-level-ul").attr("data-level")-0+1; 
                var row = {id:"",name:"",parent_id:parent_id};
        ...
        element.delegate(".form-control","blur",function(){
            ...
            data.parent_id = parentUl.attr("data-parent_id");
        ...
        function renderHeader(_dom_header){
    		...
    		dom_row.append($("<span class='textalign-center'>العمليات</span>")); 
		...
		function loadNode(loadElement,parentNode){
        	...
        	if(parentNode&&parentNode.id){
                for(var i=0;i<settings.data.length;i++){
    	        	var row = settings.data[i];
    	        	//render first level row while row.parent_id equals 0 or null or undefined
    	        	if(row.parent_id==parentNode.id){
    	        		generateTreeNode(curElement,row,curLevel);
                        //cache treenode 
                        treeData[row.id] = row;
    	        	}	        	
        ...
        function generateTreeNode(curElement,row,curLevel,isPrepend){   
            ...
            dom_ul.attr("data-parent_id",row.parent_id).attr("data-level",curLevel).attr("data-id",row.id);
